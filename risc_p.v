...
wire [31:0] Pc_Mux;
Program_Counter pc(
    .Clk(Clk), .Stall(1'b0),.Start_RISC(Clear),
    .Pc(Pc_Mux),
    .Next_Pc(Next_Pc)
);
...
pc_addimm add_imm(
    .pc(Next_Pc), .imm_value(Imm_Value),
...
alusrc_b alu_srcb( 
    .Operand2(Operand2_Reg),.Imm_Value(Imm_Value),.Pc_Imm(Add_Extend),
    .Select(Src_B),
    .Alu_b(Operand2_Alu)
);

wire [31:0] Memory_Value_Mux; 
...
store_mux store_mux(
    .sel(Store_Sel), 
    .store_value(Read_Mem),
    .store(Memory_Value_Mux)
);

load_mux load_mux(
    .sel(Load_Sel), 
    .load_value(Operand2_Reg),
    .load(Write_Data));

resultsrc_mux res_src(
    .Alu_Res(Alu_Result_Wire), .Data_Mem(Memory_Value_Mux), .Pc_plus4(Pc_Plus4),
    .result_src(Result_Src),
...
module cpu_tb;
    ...
    // Monitor output
    initial begin
        $monitor("Time = %d, Reset = %b, Out = %h, Ins=%h, funct7 =%b, rs2 =%b, rs1=%b, funct3=%b, rd=%b, opcode=%b", $time, Reset, Out,Ins,Ins[31:25],Ins[24:20],Ins[19:15],Ins[14:12],Ins[11:7],Ins[6:0]);
    end
